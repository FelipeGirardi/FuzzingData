...
df_sorted = df.sort_values('edges_not_covered', ascending=False)

edges_covered = df_sorted['edges_covered'].values
edges_total = df_sorted['edges_total'].values
edges_not_covered = df_sorted['edges_not_covered'].values
labels = df_sorted['function'].values
file_name = df_sorted['file'].iloc[0]

# Treemap 1: edges covered in green, edges not covered in red (plotly)
df_covered = df_sorted.copy()
...
fig = px.treemap(df_all, path=['file', 'function', 'edges'], values='edges', color='covered', color_discrete_map={'(?)': 'lightgrey'})
...
for i in df.index:
    diff = abs(df['edges_covered'][i] - df2['edges_covered'][i])
...
df_diffs['isDiffOrRemainder'] = ['Diff'] * len(df_diffs)
...
fig2 = px.treemap(df_plot, path=['file', 'function', 'diffs'], values='diffs', branchvalues="total", color='isDiffOrRemainder', color_discrete_map={'(?)': 'lightgrey', 'Diff': 'green', 'Remainder': 'gray'})
